...
(function( $ ) {
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.length <= 0 ) {
			...
			var div = '<div class="parentFileBox" style="width:500px;height:auto"> \
		   ...
		   var size = file.size;
			 if(size > 1024*10240){
        		...
        		var file_size = (Math.round(size*100/(1024*1024))/100).toFixed(1).toString()+'MB';
         }else {
            	var file_size = (Math.round(size*100/1024)/100).toFixed(1).toString()+'KB';
        	 }
                    	  
			var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
				<div class="mail_attachment"> \
					<div class="attachment_pic "><img style="width:18px;height:18px;" src="../assets/images/receive_mail/fujian.png" alt="附件" title="附件"></div>\
					<div class="diySuccess"></div> \
					<div class="diyFileName mail_overflow">'+file.name+'</div>\
					<input type="hidden" id="'+file.name+'" name="fileBox_'+file_id+'"  value="'+file.name+'" /> \
					<input type="hidden" name="fileOld_'+file_id+'"  value="'+file.name+'" /> \
					<input type="hidden" name="fileSize_'+file_id+'"  value="'+file_size+'" /> \
					<div class="fileSize">('+file_size+')</div> \
					<div class="diyBar display_block"> \
							<div class="diyProgress"></div> \
							<div class="diyProgressText">0%</div> \
					</div> \
					<div class="attch_delete">删除</div>\
				</div>\
			</li>';
		...
		//绑定删除事件;
		var $diyCancel = $('#fileBox_'+file_id).on('click',function(){
